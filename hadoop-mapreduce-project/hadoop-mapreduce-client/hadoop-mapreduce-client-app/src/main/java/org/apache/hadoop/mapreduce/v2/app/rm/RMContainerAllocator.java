  ...
  */
  ...
  public void scheduleAllMaps() {
    ...
    for(ContainerRequestEvent reqEvent : this.pendingMaps) {
      // For OPS, modify request host.
      String host = this.opsFilter.requestMapHostWithLocality(reqEvent.getHosts());
      System.out.println("scheduleAllMaps: modify request host from " + Arrays.toString(reqEvent.getHosts()) + " to " + host);
      if(host != "") {
        String[] hosts = {host};
        reqEvent.setHosts(hosts);
        String[] racks = {"/default-rack"};
